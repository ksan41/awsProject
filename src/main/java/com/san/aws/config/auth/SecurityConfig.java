...
import com.san.aws.domain.user.Role;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception{
        ...
        //     - "/" 등으로 지정된 URL들은 permitAll() 옵션을 통해 전체 열람 권한을 주었다.
        //     - "/api/v1/**" 주소를 가진 API는 USER권한을 가진 사람만 가능하도록 했다.

        // anyRequest
        // => 설정된 값들 이외 나머지 URL들을 나타낸다.
        //    여기에서는 authenticated()를 추가하여 나머지 URL들은 모두 인증된 사용자들에게만(로그인 한) 허용하게 했다.
        ...
        // => 로그아웃 기능에 대한 여러 설정의 진입점. 로그아웃 성공시 / 주소로 이동한다.
